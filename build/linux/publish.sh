...
# configured in the project. For a non-AOT build, turn AOT off and publish a
# trimmed, self-contained payload instead (the project turns trimming on when
# AOT is disabled).
AOT_ARGS=
if [ "$AOT" != "true" ]; then
    AOT_ARGS="-p:PublishAot=false --self-contained"
fi
