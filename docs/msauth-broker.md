...
As of GCM 3.0, where ever possible, the broker will automatically be used for
...
an application, simplifying many common authentication challenges and provides
a more seamless and secure authentication experience.
...
in the _top right_ of the window, the select **Remove account from this
device**:
...
registration on Linux devices, including de-registation using the `--unregister`
option.
...
functionality on Windows and comes _built-in_ of **Windows 10 version 1703**
...
The [Microsoft Intune app][intunebroker-linux] app acts as the authentication
...
GCM can be configured automatically use the default or current OS account for
