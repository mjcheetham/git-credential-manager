...
public class GitResponse
{
    ...
    /// <summary>
    /// Out-of-band metadata about how the response was produced. Distinct
    /// from <see cref="AdditionalProperties"/> (which is for arbitrary
    /// extra wire-protocol output) and from <see cref="State"/> (which is
    /// the negotiated <c>state</c> protocol channel) -- this is for
    /// diagnostics and usage survey that never reach Git itself.
    /// </summary>
    /// <remarks>
    /// Always non-null. Mutate the inner properties via the standard
    ...
    /// </code>
    /// </remarks>
    public GitResponseMetadata Metadata { get; set; } = new GitResponseMetadata();
...
/// <summary>
/// Out-of-band, never-emitted-to-Git metadata attached to a
/// <see cref="GitResponse"/>. Carries information about *how* the credential
/// was produced for the benefit of diagnostics, usage survey, and downstream
/// in-process policy.
/// </summary>
/// <remarks>
/// New usage-survey fields belong here rather than as top-level
/// properties on <see cref="GitResponse"/> so the response shape stays
/// focused on the wire-protocol contract.
/// </remarks>
public class GitResponseMetadata
{
    /// <summary>
    /// <see langword="true"/> when the credential was returned from the OS
    /// credential store unchanged, <see langword="false"/> when it was
    /// freshly generated by the provider. Meaningless when no credential is
    /// being returned (response was cancelled or yielded).
    /// </summary>
    public bool FromCache { get; set; }

    /// <summary>
    /// Identifier for the authentication mechanism the provider used when
    /// freshly generating the credential, or <see langword="null"/> when
    /// the provider did not classify the mechanism or the credential was
    /// returned from cache (in which case <see cref="FromCache"/> is
    /// <see langword="true"/>).
    /// </summary>
    /// <remarks>
    /// Values are provider-specific strings (e.g. <c>"oauth-broker"</c>,
    /// <c>"basic"</c>, <c>"managed-identity"</c>) and form part of the
    /// usage survey schema. Adding a new value is a breaking change for the
    /// closed allow-list on the published aggregates pipeline.
    /// </remarks>
