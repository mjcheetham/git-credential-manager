...
namespace Microsoft.AzureRepos
{
    public class AzureReposHostProvider : DisposableObject, IHostProvider, IConfigurableComponent, ICommandProvider
    {
        ...
        ProviderCommand ICommandProvider.CreateCommand()
        {
            ...
            // clear-cache
            //
            var clearCacheCmd = new Command("clear-cache", "Clear the Entra authority cache");
            clearCacheCmd.SetHandler(ClearCacheCmd);

            //
            // login [--tenant <id|domain>]
            //
            var loginCmd = new Command("login", "Sign in to a Entra account and add it to the credential cache");
            ...
            // logout (<account> | --all)
            //
            var logoutCmd = new Command("logout", "Remove an Entra account from the credential cache");
            var logoutAccountArg = new Argument<string>("account", "Account to remove (UPN or HomeAccountId)")
            {
                Arity = ArgumentArity.ZeroOrOne
            };
            var logoutAllOpt = new Option<bool>("--all", "Remove every cached Entra account");
            ...
            // list
            //
            var listCmd = new Command("list", "List Entra accounts in the credential cache");
